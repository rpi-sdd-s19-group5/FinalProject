...
class MyThread(threading.Thread):
    ...
    def run(self):
        ...
        url = "http://catalog.rpi.edu/content.php?catoid=18&catoid=18&navoid=444&filter%5Bitem_type%5D=3&filter" \
              "%5Bonly_active%5D=1&filter%5B3%5D=1&filter%5Bcpage%5D=" + str(
            self.num) + "#acalog_template_course_filter"
        chrome.get(url)
        print(url)
        ...
        for course_link in trs:
            ...
            try:
                ...
                content = WebDriverWait(course_link, 10).until(
                    expected_conditions.visibility_of_element_located((By.CLASS_NAME, "ajaxcourseindentfix"))
                )
                ...
                # threadLock.release()
                print("thread #" + str(self.num) + str(course["title"]))
